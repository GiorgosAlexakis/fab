...
// so an interrupted run leaves the database at a migration boundary rather than
// halfway through one.
func Apply(ctx context.Context, db storage.Beginner, component string, migrations []Migration) ([]Migration, error) {
	if err := createBookkeepingTable(ctx, db); err != nil {
		return nil, err
	}

	// One migrating process at a time: two concurrent runs would both see the
	// same pending list and both try to create the same tables.
