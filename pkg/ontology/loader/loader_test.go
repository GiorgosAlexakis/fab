...
`)
	...
	// The resolver's order, which the loader must follow rather than re-derive.
	// meta-devops is active and ships no schema, so it drops out silently.
	options := Options{Root: root, Layers: []string{"meta-marine", "meta-devops", "meta-core"}}

	sources, err := LoadFoundry(options)
	if err != nil {
		t.Fatalf("LoadFoundry() failed: %v", err)
	}

	wantLayers := []string{"meta-marine", "meta-core", "app"}
	...
	for i, want := range wantLayers {
		if sources[i].Layer != want {
			t.Errorf("layer %d = %q, want %q (the resolver's order, then app)", i, sources[i].Layer, want)
		}
...
// A layer directory that foundry.yaml does not activate contributes nothing.
// Otherwise an inactive layer's types could satisfy a cross-layer reference that
// is supposed to fail.
func TestLoadFoundrySkipsInactiveLayers(t *testing.T) {
	root := t.TempDir()
...
`)
	...
	if len(sources) != 1 || sources[0].Layer != "app" {
		t.Fatalf("got %+v, want only the app layer", documentLayers(sources))
	}
}
...
func documentLayers(sources []compiler.LayerSource) []string {
	layers := make([]string, 0, len(sources))
	for _, source := range sources {
		layers = append(layers, source.Layer)
	}
	return layers
}
