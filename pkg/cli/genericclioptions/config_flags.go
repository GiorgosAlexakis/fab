...
// DefaultRegistryURL is where the registry server listens in the local
// docker compose deployment, which is the common case for a developer running
// schema commands.
...
// ToRegistryURL resolves the base URL of the registry server from the flag or
// the environment. The registry is reached over its API rather than by
// connecting to its database, so that only the server holds database
// credentials.
